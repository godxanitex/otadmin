...
<?php
    $list = array();
    exec("ls $ot_root/data/logs/chat/*.log", $list); 
    for ($i=0; $i<sizeof($list); $i++)
    {
        //echo '<a href="#modal'.$i.'" data-toggle="modal">' . $list[$i] . '</a><br />';
        echo '<a href="#" id="chatlaunch'.$i.'">' . $list[$i] . '</a><br />';
?>
<!-- Modal -->
<div id="<?php echo "chatmodal$i"; ?>" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
    <h3 id="myModalLabel"><?php echo $list[$i]; ?></h3>
  </div>
  <div class="modal-body">
    <div id="chatloading<?php echo $i; ?>" style="text-align: center; margin: auto;"><img src="loading.gif" /></div>
    <p><pre id="chatmodal_text<?php echo $i; ?>"><?php //echo htmlentities(file_get_contents($list[$i])); ?></pre></p>
  </div>
...
<script>
$("#chatlaunch<?php echo $i; ?>").click(function(e){
    e.preventDefault();
    $("#chatloading<?php echo $i; ?>").show();
    $("#<?php echo "chatmodal$i"; ?>").modal('show');
    $.ajax({
        url: 'ajax_file.php',
        data: {
        ...
        $("#chatloading<?php echo $i; ?>").hide();
        $("#chatmodal_text<?php echo $i; ?>").text(data);
    });
});
</script>
<?php
